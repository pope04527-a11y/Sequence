...
export default function CustomerServiceModal({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch("https://sequence-backend-5v07.onrender.com/service-links.json?ts=" + Date.now())
      .then((res) => res.json())
      .then((data) => {
        setLinks({
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0, padding: "6px 0" }}>
          ...
          <button
            onClick={() => {
              ...
              const chatUrl = `https://sequence-cs-chat.onrender.com/?user=${encodeURIComponent(username)}`;
